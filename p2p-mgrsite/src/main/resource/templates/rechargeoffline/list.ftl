				...
				<div class="panel panel-default">
					<table class="table">
						...
						<#list pageResult.list as info>
							<tr>
								<td>${info.applierId.username}</td>
								<td>${info.tradeCode}</td>
								<td>${info.tradeTime?string('yyyy-MM-dd')}</td>
								<td>${info.amount}</td>
								<td>
									<script>
										var str="("+SITE_BANK_TYPE_NAME_MAP["${info.bankInfoId.bankName}"]+")${info.bankInfoId.accountNumber}${info.bankInfoId.bankForkName}";
										document.write(str);
									</script>
								</td>
								<td>${info.stateDisplay}</td>
								<td>${(info.auditorId.userName)!""}</td>
								<td>
									<#if (info.state == 0)>
